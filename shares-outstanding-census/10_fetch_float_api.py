"""Fetch SEC's structured XBRL record of dei:EntityPublicFloat for every
in-scope CIK (the companyconcept API) — the secondary XBRL source for the
public-float ladder, mirroring script 5 for shares outstanding.

The filing's own inline XBRL (script 8) is the primary validation source;
this API adds coverage where the primary document carries no parseable facts
and an independent cross-check elsewhere. Each fact carries the accession
number ("accn") of the filing that reported it, so rows join our population
directly.

Output: float_api_facts_{year}.csv (accession, cik, value, end, fy, fp).
Raw JSON responses are cached per CIK under DATA_DIR/cache/xbrl_api_float/;
...
api_cache = os.path.join(directory, "cache", "xbrl_api_float")
pop_path = os.path.join(directory, "population_%d.csv" % year)
out_path = os.path.join(directory, "float_api_facts_%d.csv" % year)
        ...
        for fact in unit_rows:
            if fact.get("accn") in accessions and fact.get("val") is not None:
                rows.append({
                    ...
                    # monetary: keep sub-dollar precision when present
                    "value": ("%d" % fact["val"])
                             if float(fact["val"]) == int(fact["val"])
                             else str(fact["val"]),
                    "end": fact.get("end", ""),
